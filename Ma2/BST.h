...
public:
	...
	/* MA TODO: Implement */
	~BST() {
		destroyTree(_root); // Helper function from cpts 122 pa 6
		cout << "Destructing tree" << endl;
	}
	//TAKEN FROM PREVIOUS 122 CODE HELPER FUNCTION
	...
	BST(const BST& other) : _root(nullptr) {
		...
		if (other._root != nullptr) {
			copyHelper(_root, other._root);
		}
	}
	//copy helper function recursivly vists all nodes and copys them to destionation
	void copyHelper(Node<T>*& dest, Node<T>* src) {
		if (src == nullptr) {
			dest = nullptr;
			return;
		}
	...
	/* Move constructor */
	/* MA TODO: Implement */
	//Doesnt need to allocate new node data becuase by properities of move
	BST(BST&& other) : _root(nullptr) {
		if (this->_debug) {
			cout << " Move constructor called " << endl;
		}

		this->_root = move(other._root);
		other._root = nullptr;
	}
	...
	BST& operator=(BST& other) {
		...
		//checks for data becuase we arent constructing a new object
		if (_root != nullptr) {
			destroyTree(_root);
			_root = nullptr;
		}
